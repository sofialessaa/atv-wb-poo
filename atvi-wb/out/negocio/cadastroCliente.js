...
var CadastroCliente = /** @class */ (function (_super) {
    ...
    CadastroCliente.prototype.cadastrar = function () {
        ...
        do {
            var generoString = this.entrada.receberTexto("Por favor informe o g\u00EAnero do cliente (F/M/Outro): ").toUpperCase();
            if (generoString === 'M' || generoString === 'F' || generoString === 'Outro') {
                genero = generoString; // Convertendo string para Genero
            }
            else {
                console.log('Opção inválida! Por favor, escolha entre M, F ou Outro.');
            }
        } while (!genero);
        /* cadastro do cliente */
